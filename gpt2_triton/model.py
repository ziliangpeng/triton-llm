...
from triton_llm.kernels.add import add as _gpu_add
...
from triton_llm.kernels.gemm import gemm as _gpu_gemm
...
def gemm(x: np.ndarray, w: np.ndarray) -> np.ndarray:
    """Compatibility wrapper: numpy in -> numpy out using GPU-only GEMM kernel."""
    x = np.require(x, dtype=np.float32, requirements=["C_CONTIGUOUS"])
    w = np.require(w, dtype=np.float32, requirements=["C_CONTIGUOUS"])
    out_dev = _gpu_gemm(gpu.to_device(x), gpu.to_device(w))
    gpu.synchronize()
    return gpu.to_host(out_dev)


def add(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    """Compatibility wrapper: numpy in -> numpy out using GPU-only add kernel."""
    x = np.require(x, dtype=np.float32, requirements=["C_CONTIGUOUS"])
    y = np.require(y, dtype=np.float32, requirements=["C_CONTIGUOUS"])
    out_dev = _gpu_add(gpu.to_device(x), gpu.to_device(y))
    gpu.synchronize()
    return gpu.to_host(out_dev)
